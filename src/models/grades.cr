  ...
  class WeekOfEvents
    ...
    def missed_rehearsal?
      @events_with_attendance.any? do |(event, attendance)|
        event.type == Models::Event::REHEARSAL && attendance.try &.deny_credit?
      end
    end

    def first_missed_sectional
      @events_with_attendance.find do |(event, attendance)|
        event.type == Models::Event::SECTIONAL && attendance.try &.deny_credit?
      end
    end
    ...
    def sectionals
      @events_with_attendance.select do |(event, attendance)|
        event.type == Models::Event::SECTIONAL
      end
    end
    ...
    def attended_volunteer_gig(event, attendance)
      attendance.try &.did_attend &&
        ...
        event.type == Models::Event::VOLUNTEER_GIG &&
        event.gig_count
    end
  end
end
